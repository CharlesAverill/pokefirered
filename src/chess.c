...
static const u16 sPieceImageSizes[][4] = {
    [ST_OAM_SQUARE] = {
        [ST_OAM_SIZE_0] = 0x020,
        [ST_OAM_SIZE_1] = 0x080,
        [ST_OAM_SIZE_2] = 0x200,
...
#define PIECE_SIZE SPRITE_SIZE(32x32)
#define PIECE_SHAPE SPRITE_SHAPE(32x32)
#define PALETTE_TAG_PIECE_WHITE 0x1000
#define PALETTE_TAG_PIECE_BLACK 0x1001

static const struct OamData sPieceOamData = {
    .x = 0,
    .y = 0,
 ...
 */
static const u32 sSampleUiTiles[] = INCBIN_U32("graphics/chess/tiles.4bpp.lz");
static const u32 sBoardTiles[] = INCBIN_U32("graphics/chess/board_tiles.4bpp.lz");

/*
 * I created this tilemap in TilemapStudio using the above tile PNG. I highly recommend TilemapStudio for exporting maps
 * like this.
 */
static const u32 sSampleUiTilemap[] = INCBIN_U32("graphics/chess/tilemap.bin.lz");
static const u32 sBoardTilemap[] = INCBIN_U32("graphics/chess/board_tiles.bin.lz");

static const u32 sPieceSprites[] = INCBIN_U32("graphics/chess/pieces.4bpp.lz");
static const u32 sPiecePalettes[] = INCBIN_U32("graphics/chess/pieces.gbapal.lz");
 ...
 * problems.
 */
static const u16 sSampleUiPalette[] = INCBIN_U16("graphics/chess/00.gbapal");
static const u16 sBoardPalette[] = INCBIN_U16("graphics/chess/board_tiles.gbapal");
...
static void Chess_DrawPawn(void);
...
static void Task_SampleUiMainInput(u8 taskId)
{
    ...
    if (JOY_REPT(DPAD_DOWN))
    {
        ...
        DestroyMonIcon(&gSprites[sSampleUiState->monIconSpriteId]);
...
static bool8 SampleUi_LoadGraphics(void)
{
    ...
    case 1:
        ...
        if (FreeTempTileDataBuffersIfPossible() != TRUE)
        {
             ...
             */
            // LZDecompressWram(sBoardTilemap, sBg1TilemapBuffer);
    ...
    case 2:
         ...
         */
        LoadPalette(GetUserFrameGraphicsInfo(gSaveBlock2Ptr->optionsWindowFrameType)->palette, BG_PLTT_ID(15), PLTT_SIZE_4BPP);
        sSampleUiState->loadState++;
    default:
        sSampleUiState->loadState = 0;
        return TRUE;
...
static void SampleUi_InitWindows(void)
{
    ...
    /*
     * Copy (well, schedule to copy) each window into VRAM using DMA3 under the hood. The COPYWIN_BOTH argument means we
     ...
     */
    CopyWindowToVram(WIN_UI_HINTS, COPYWIN_BOTH);
    CopyWindowToVram(WIN_MON_INFO, COPYWIN_BOTH);
}
...
static void SampleUi_PrintUiButtonHints(void)
{
    ...
    /*
     * Use the text printing system to add text to this window. We set the speed value to 0 to tell the
     ...
     * occuring (for a technical reason explained below). Try changing the speed parameter 0 to 0 (which
     * tells the text printer to copy to VRAM on the next VBlank) and observe the slight flicker that occurs.
     *
     * The reason you see flickering when drawing lots of text without setting 0 is because without this,
...
static void SampleUi_PrintUiMonInfo(void)
{
    ...
    case MODE_INFO:
        ...
        GetSpeciesName(gStringVar2, speciesId);
        StringExpandPlaceholders(gStringVar3, sText_SampleUiMonInfoSpecies);
        GetSpeciesPokedexDescription(gStringVar4, speciesId);
    ...
    case MODE_STATS:
        AddTextPrinterParameterized4(WIN_MON_INFO, FONT_SHORT, 5, 3, 0, 0, sSampleUiWindowFontColors[FONT_BLACK],
            0, sText_SampleUiMonStats);
        break;
    case MODE_OTHER:
        AddTextPrinterParameterized4(WIN_MON_INFO, FONT_SHORT, 5, 3, 0, 0, sSampleUiWindowFontColors[FONT_BLACK],
            0, sText_SampleUiMonOther);
        break;
    default:
        break;
    }
...
static void SampleUi_DrawMonIcon(u16 dexNum)
{
     ...
     */
    // CreateMonIcon(species, SpriteCallbackDummy, 24, 32, 0, personality, 0);
    // SafeLoadMonIconPalette(speciesId);
    LoadMonIconPalettes();
    sSampleUiState->monIconSpriteId = CreateMonIcon(SPECIES_ABRA, SpriteCallbackDummy, 0x38, 0x28, 0, 0, 1);
    gSprites[sSampleUiState->monIconSpriteId].oam.priority = 3;

    AGBPrintf("Before copying to VRAM\n");

    src = gMonFrontPicTable[species].data;
    dst = (void *)(VRAM + 0x10000 + gSprites[sSampleUiState->monIconSpriteId].oam.tileNum * 32);
    LZDecompressVram(src, dst);

    AGBPrintf("After copying to VRAM: %d\n", sSampleUiState->monIconSpriteId);
    AGBPrintFlush();
}
...
static void Chess_DrawPawn(void)
{
    ...
    struct SpriteFrameImage image = { NULL, sPieceImageSizes[PIECE_SHAPE][PIECE_SIZE] };

    struct SpriteTemplate spriteTemplate =
    {
        .tileTag = PALETTE_TAG_PIECE_WHITE,
        .paletteTag = PALETTE_TAG_PIECE_WHITE,
        .oam = &sPieceOamData,
    ...
    struct CompressedSpriteSheet sCompressedPieceSheet =
	{
		sPieceSprites, 1024, PALETTE_TAG_PIECE_WHITE
	};
	struct CompressedSpritePalette sCompressedPiecePalette =
	{
		sPiecePalettes, PALETTE_TAG_PIECE_WHITE
	};

    LoadCompressedSpriteSheet(&sCompressedPieceSheet);
	LoadCompressedSpritePalette(&sCompressedPiecePalette);
    spriteId = CreateSprite(&spriteTemplate, MON_ICON_X, MON_ICON_Y, 0);
    gSprites[spriteId].animPaused = TRUE;
    gSprites[spriteId].animBeginning = FALSE;
    gSprites[spriteId].images = (const struct SpriteFrameImage *)sPieceSprites;
    gSprites[spriteId].data[0] = 0;
	gSprites[spriteId].data[1] = 0;
	gSprites[spriteId].data[2] = 0;
    sSampleUiState->monIconSpriteId = CreateMonIcon(SPECIES_ABRA, SpriteCB_MonIcon, MON_ICON_X, MON_ICON_Y, 4, 0, FALSE);
}
